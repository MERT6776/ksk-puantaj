...
LANGS = {
    "TR": {
        ...
        "legend": "KISALTMALAR VE ANLAMLARI",
        "theme": "Tema Seçimi", "month_title": "PERSONEL PUANTAJI", "overtime": "SAAT",
        "logout": "ÇIKIŞ YAP", "subject": "Konu", "err": "Bilgiler hatalı, tekrar deneyin.",
        "topic_opts": ["Seçiniz...", "Puantaj İtirazı", "Mesai İtirazı", "Diğer"],
        "summary": "AY ÖZETİ",
        "verify_title": "GÜVENLİK DOĞRULAMASI",
        "verify_desc": "Robot olmadığınızı doğrulamak için aşağıdaki kodu giriniz.",
        "verify_field": "DOĞRULAMA KODU", "verify_btn": "DOĞRULA VE GİR",
        "verify_err": "Kod hatalı, lütfen tekrar deneyin.", "new_code": "🔄 Yeni Kod",
        "back": "← Geri",
        ...
        "mail_ready": "İtirazınız hazırlandı. Aşağıdaki butona basınca mail uygulamanız açılır.",
        "open_mail": "MAİL UYGULAMASINI AÇ"
    },
    "EN": {
        ...
        "logout": "LOGOUT", "subject": "Subject", "err": "Invalid credentials, please try again.",
        "topic_opts": ["Select...", "Payroll Objection", "Overtime Objection", "Other"],
        "summary": "MONTHLY SUMMARY",
        "verify_title": "SECURITY CHECK",
        "verify_desc": "Enter the code below to verify you are not a robot.",
        "verify_field": "VERIFICATION CODE", "verify_btn": "VERIFY & ENTER",
        "verify_err": "Wrong code, please try again.", "new_code": "🔄 New Code",
        "back": "← Back",
        "disc_title": "NOTICE",
        ...
        "mail_ready": "Your appeal is ready. Tap the button below to open your mail app.",
    ...
    "UZ": {
        ...
        "logout": "CHIQISH", "subject": "Mavzu", "err": "Ma'lumot noto'g'ri, qayta urinib ko'ring.",
        "topic_opts": ["Tanlang...", "Puantaj e'tirozi", "Ish vaqti e'tirozi", "Boshqa"],
        "summary": "OYLIK HISOBOT",
        "verify_title": "XAVFSIZLIK TEKSHIRUVI",
        "verify_desc": "Robot emasligingizni tasdiqlash uchun quyidagi kodni kiriting.",
        "verify_field": "TASDIQLASH KODI", "verify_btn": "TASDIQLASH VA KIRISH",
        "verify_err": "Kod noto'g'ri, qayta urinib ko'ring.", "new_code": "🔄 Yangi Kod",
        "back": "← Orqaga",
        "disc_title": "MA'LUMOT",
        ...
        "mail_ready": "E'tirozingiz tayyor. Quyidagi tugmani bosing, pochta ilovangiz ochiladi.",
        "open_mail": "POCHTA ILOVASINI OCHISH"
    }
}

# KISALTMALAR (HTÇ = Pazar Çalışması, HÇ yok)
STATUS_MAP = {
    "N": "Normal Çalışma", "HTÇ": "Pazar Çalışması", "HT": "Hafta Tatili",
    "B": "Bayram Tatili", "BÇ": "Bayramda Çalışma", "Üİ": "Personel Çalışmadı"
}

# AY / GÜN İSİMLERİ (dile göre)
AYLAR = {
...
# GÖREV ÇEVİRİLERİ (bilinmeyen görev Türkçe kalır — buraya ekleyebilirsin)
GOREV_MAP = {
...
# ------------------------------------------------------------------
# RENK PALETLERİ (2 yeni tema eklendi)
# ------------------------------------------------------------------
THEMES = {
    ...
    "Gece Mavisi": {"bg_grad_1": "#0a1128", "bg_grad_2": "#1c3a5e", "card_bg": "rgba(255,255,255,0.06)",
    ...
    "Zümrüt Koyu": {"bg_grad_1": "#06231b", "bg_grad_2": "#0b3d2e", "card_bg": "rgba(255,255,255,0.06)",
...
# ------------------------------------------------------------------
def init_state():
    d = {'lang': "TR", 'theme': "Kurumsal Koyu", 'logged_in': False, 'awaiting_verify': False,
...
.kod-box {{ text-align: center; font-family: 'Courier New', monospace; font-size: 40px; font-weight: 900; letter-spacing: 12px;
    color: {T["accent"]}; background: {T["card_bg"]}; border: 2px dashed {T["accent"]}; border-radius: 14px; padding: 16px 10px; margin-bottom: 16px; }}
...
.hafta-baslik {{ font-size: 12.5px; font-weight: 900; letter-spacing: 1px; color: {T["accent"]}; text-transform: uppercase; margin: 4px 0 7px; }}
...
/* Ana aksiyon butonları (gradyan) */
.stButton > button, .stLinkButton > a, .stForm [data-testid="stFormSubmitButton"] > button {{ width: 100%; border-radius: 12px !important; border: none !important;
...
/* İkincil butonlar = hafta aç/kapa (sade) */
.stButton > button[kind="secondary"] {{ background: {T["card_bg"]} !important; color: {T["text_main"]} !important;
    border: 1px solid {T["card_border"]} !important; box-shadow: none !important; text-align: left; }}
...
def get_status_class(durum):
    ...
    return {"N": "status-n", "HTÇ": "status-htc", "HT": "status-ht",
            "BÇ": "status-bc", "B": "status-b", "Üİ": "status-ui"}.get(durum, "status-default")
...
if not st.session_state['logged_in'] and not st.session_state['awaiting_verify']:
    ...
    st.selectbox(L['lang'], ["TR", "EN", "UZ"], key='lang')
    st.selectbox(L['theme'], list(THEMES.keys()), key='theme')
    ...
    if st.button(L['login'], type="primary"):
        if df is not None:
            ...
            if not res.empty:
                ...
                st.session_state['verify_code'] = f"{random.randint(0, 9999):04d}"  # 4 haneli, sadece rakam
                st.rerun()
            else:
                st.error("❌ " + L['err'])
...
# ------------------------------------------------------------------
# EKRAN 2 — DOĞRULAMA (4 haneli rastgele kod)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
    ...
    with ust1:
        st.selectbox(L['lang'], ["TR", "EN", "UZ"], key='lang', label_visibility="collapsed")
    with ust2:
        st.selectbox(L['theme'], list(THEMES.keys()), key='theme', label_visibility="collapsed")
    with ust3:
        if st.button("🚪 " + L['logout'], type="primary", use_container_width=True):
            for k in ['logged_in', 'awaiting_verify', 'user_data', 'pending_user', 'itiraz_ready']:
                st.session_state[k] = False if k in ('logged_in', 'awaiting_verify', 'itiraz_ready') else None
            st.rerun()
    ...
    # İTİRAZ MERKEZİ (form ile — bilgiler garanti maile gider)
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    ...
    if gonder:
        ...
        mail_govde = (
            f"Sicil No: {row_g['FİORİ NO']}\n"
            f"Ad Soyad: {row_g['AD SOYAD']}\n"
            f"Görevi: {row_g['GÖREVİ']}\n"
            f"Konu: {konu}\n"
            f"Not: {notunuz}"
        )
