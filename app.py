...
# 2. DİL VE VERİ SÖZLÜĞÜ (KULLANICI ADI MÜHÜRLÜ)
LANGS = {
    "TR": {
        ...
        "phys_days": "Fiziki Gün", "total_over": "TOPLAM MESAİ", "week": "HAFTA",
    ...
    "EN": {
        ...
        "phys_days": "Physical Days", "total_over": "TOTAL OVERTIME", "week": "WEEK",
    ...
    "UZ": {
        ...
        "phys_days": "Ishlagan Kun", "total_over": "UMUMIY ISH VAQTI", "week": "HAFTA",
...
# 2.1 TEMA SÖZLÜĞÜ (SENİN SEÇTİĞİN LÜKS RENKLER)
THEMES = {
    "Kurumsal Koyu": {
        ...
        "accent": "#38bdf8", "accent_2": "#f59e0b", "clock": "#f8fafc", "input_bg": "rgba(255,255,255,0.06)",
        "input_text": "#ffffff", "btn_text": "#ffffff", "shadow": "0 10px 30px rgba(0,0,0,0.30)", "overlay": "rgba(2, 6, 23, 0.82)"
    },
    "Açık Kurumsal": {
        "bg_grad_1": "#ffffff", "bg_grad_2": "#e2e8f0", "card_bg": "rgba(255,255,255,0.92)",
        "card_border": "rgba(15,23,42,0.10)", "text_main": "#0f172a", "text_soft": "#475569",
        "accent": "#1d4ed8", "accent_2": "#f59e0b", "clock": "#0f172a", "input_bg": "#ffffff",
        "input_text": "#0f172a", "btn_text": "#ffffff", "shadow": "0 10px 25px rgba(15,23,42,0.08)", "overlay": "rgba(255,255,255,0.78)"
    },
    "Endüstriyel Şantiye": {
        "bg_grad_1": "#111827", "bg_grad_2": "#374151", "card_bg": "rgba(17,24,39,0.82)",
        "card_border": "rgba(250,204,21,0.22)", "text_main": "#f9fafb", "text_soft": "#d1d5db",
        "accent": "#facc15", "accent_2": "#f97316", "clock": "#facc15", "input_bg": "rgba(255,255,255,0.05)",
        "input_text": "#ffffff", "btn_text": "#111827", "shadow": "0 12px 30px rgba(0,0,0,0.38)", "overlay": "rgba(10, 10, 10, 0.72)"
    },
    "Premium Cam": {
        ...
        "accent": "#60a5fa", "accent_2": "#fbbf24", "clock": "#ffffff", "input_bg": "rgba(255,255,255,0.08)",
...
st.markdown(f"""
    ...
    .block-container {{ padding-top: 1.8rem !important; padding-bottom: 2rem !important; max-width: 920px !important; }}
    #live-clock {{ text-align: right; color: {T["clock"]}; font-family: 'Courier New', monospace; font-weight: 900; font-size: 22px; letter-spacing: 1.5px; padding: 10px 4px; text-shadow: 0 2px 8px rgba(0,0,0,0.35); }}
    ...
    .shift-container {{ width: 100%; background: rgba(255,255,255,0.08); border-radius: 999px; height: 18px; margin: 14px 0; border: 1px solid {T["card_border"]}; overflow: hidden; }}
    ...
    .day-item {{ text-align: center; font-weight: 900; border-radius: 14px; padding: 12px 6px; color: white; min-height: 95px; box-shadow: 0 8px 16px rgba(0,0,0,0.18); transition: transform 0.15s ease, box-shadow 0.15s ease; }}
    ...
    /* MESAİ ETİKETİ (V31'İN JİLET GİBİ FORMATI) */
    .overtime-label {{
    ...
    .stButton > button, .stLinkButton > a {{ width: 100%; border-radius: 12px !important; border: none !important; font-weight: 800 !important; min-height: 46px; background: linear-gradient(90deg, {T["accent"]}, {T["accent_2"]}) !important; color: {T["btn_text"]} !important; box-shadow: 0 10px 22px rgba(0,0,0,0.18); }}
    .stButton > button:hover, .stLinkButton > a:hover {{ filter: brightness(1.05); transform: translateY(-1px); }}
    hr {{ border: none; height: 1px; background: linear-gradient(90deg, transparent, {T["card_border"]}, transparent); margin: 18px 0; }}
    .mert-signature {{ position: fixed; bottom: 12px; left: 15px; font-size: 13px; font-weight: 900; color: {T["text_soft"]}; opacity: 0.7; letter-spacing: 2px; z-index: 1000; }}
...
# 4. VERİ MOTORU & ÇELİK TARİH ÇÖZÜCÜ (V31 MANTIĞI KORUNDU)
@st.cache_data
def load_data():
    try:
...
def filyos_date_engine(t_col):
    """Tarihleri milimetrik çözer ve ay bilgisini döndürür. (V31 Motoru)"""
    try:
    ...
    for h_no, i in enumerate(range(0, len(t_cols), 7), 1):
        ...
        with st.expander(f"📁 {L['week']} {h_no} {L['week_suffix']}"):
            ...
            for t_col in hafta:
                ...
                # 🚀 ŞUBAT AYI MESAI GIZLEME MANTIĞI & TEMİZ ETİKET (V31'den Korundu)
                ...
                st.markdown(f'''
                    <div class="day-item {cls}">
                        ...
                        <span style="font-size:9px; opacity:0.85;">{g_adi}</span><br>
                        {mesai_html}
                    </div>
                ''', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
